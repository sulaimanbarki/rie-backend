            ...
            <div class="row" id="basic-table">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="post" action="add-service-files" class="row" ref="form"
                                enctype="multipart/form-data" v-if="$cookies.get('permissions').includes('Write')">
                                @csrf
                                <input class="form-control" type="hidden" name="service_id" value="{{ $id }}">
                                <div class="col-12">
                                    @if ($errors->has('file'))
                                ...
                                <div class="col-12">
                                    ...
                                    <fieldset class="form-group">
                                        <div class="custom-file">
                                            <input type="text" value="{{ old('file_name') }}" name="file_name"
                                                class="form-control">
                                        </div>
                                    </fieldset>
                                </div>
                            ...
                            <table id="table-extended-success" class="table mb-0 dataTable no-footer" role="grid">
                                <thead>
                                    <tr role="row">
                                        <th class="sorting_asc" tabindex="0" aria-controls="table-extended-success"
                                            rowspan="1" colspan="1" aria-sort="ascending"
                                            aria-label="campaign: activate to sort column descending"
                                            style="width: 216.797px;">S#</th>
                                        <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="category"
                                            style="width: 165.406px;">File Name</th>
                                        <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="category"
                                            style="width: 165.406px;">File</th>
                                        <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="action"
                                            style="width: 84.6094px;">action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @php
                                    $s = 1;
                                    @endphp
                                    @foreach ($servicefiles as $item)
                                    <tr role="row" class="odd">
                                        <td class="text-bold-600 pr-0 sorting_1">{{ $s++ }}</td>
                                        <td>
                                            {{ $item->servicefile_name }}
                                        </td>
                                        <td>
                                            <a href="{{$item->servicefile_link}}" download target="_blank">Download
                                                File</a>
                                        </td>
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a class="dropdown-item"
                                                        href="delete-service-files/{{$item->servicefile_id}}"
