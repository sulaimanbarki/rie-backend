       ...
       <div class="col-md-12">
            <fieldset class="form-group">
                <label :class="[errors.link_type ? 'danger' : '']" v-text="errors.link_type ? errors.link_type[0] : 'Footer Section *'"></label>
                 <v-select :options="linkstype" v-model="link_type" label="title"></v-select>
            </fieldset>
        </div>
...
export default {
    ...
    methods: {
        async create () {
            ...
            try {
                const { data } = await axios.post(`${this.$base}footerlink-mgt`, {
                ...
                this.isOpen = false
                this.link_type = this.link = this.link_title = ''
